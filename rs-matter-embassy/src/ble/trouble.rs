...
//!
//! Two entry points, differing only in who owns the `trouble-host` stack:
//!
//! * [`TroubleBtpGattPeripheral`] builds and owns one, and is a ready-made `GattPeripheral` to
//!   hand to the Matter stack. This is what you want when Matter is the only user of the BLE
//!   controller.
//! * [`run_btp_gatt_service`] serves BTP on a `Peripheral` borrowed from a stack the caller
//!   built, for a device that needs the BLE central role alongside Matter.
//!
//! The former is a thin wrapper around the latter: it builds a stack and drives its `Runner`.
...
/// The largest BTP frame the service will assemble, and hence the required size of the
/// indication buffer passed to [`run_btp_gatt_service`].
    ...
    C: Controller,
{
    ...
    pub async fn run(
        ...
        // Serving the service is the same work whoever owns the host; the only thing this
        // flavour adds is driving the `Runner` of the stack it just built alongside it.
        let mut ble_task = pin!(run_ble(runner));
        let mut service_task = pin!(run_btp_gatt_service(
            &mut peripheral,
            ind_buf,
            btp,
            service_name,
        ...
        select(&mut ble_task, &mut service_task).coalesce().await?;
...
///
/// This is the whole of the peripheral's behaviour bar the host itself: advertise, accept a
/// connection, bind the Matter attribute server to it, and pump BTP until the peer disconnects or
/// unsubscribes - then advertise again.
///
/// Use this when something other than Matter also needs the BLE controller. A controller can back
/// only one `trouble-host` stack, so a device that needs the central role too - a bridge polling
/// its own BLE sensors, say - cannot let [`TroubleBtpGattPeripheral`] build a second one. Such a
/// device builds one multirole stack itself, keeps `Stack::runner()` and `Stack::central()`, and
/// passes `Stack::peripheral()` here.
///
/// Note what this does *not* do: drive the host's `Runner`. The caller owns the stack, so the
/// caller is already polling it. [`TroubleBtpGattPeripheral::run`] builds its own stack and
/// therefore runs both.
///
/// `ind_buf` must be at least [`MAX_MTU_SIZE`] bytes, and is where outgoing BTP frames are
/// assembled before being indicated on C2.
